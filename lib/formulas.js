...
//  Utility calculator Start-----------

// /lib/formulas/ageCalculator.js
...
export function calculateAge({ dob }) {
  ...
  if (birthDate > today) {
    return {
      ...
      breakdown: {},
      stats: {},
      meta: { error: "Future date not allowed" },
    };
  }
...
//  Utility calculator End------------------
// Astrology formulas START---------

export function calculateCareerAstrology({ name, lifePathNumber, rashi }) {
  ...
  const num = Number(lifePathNumber);
  const safeNum = num >= 1 && num <= 9 ? num : 1;

  return {
    primary: careerByLifePath[safeNum],
    secondary: careerByRashi[rashi] || "General career guidance",
    meta: {
      name: name || "User",
      number: safeNum,
      rashi,
...
export function calculateLuckyColor({ name, luckyNumber }) {
  ...
  const num = Number(luckyNumber);
  const safeNumber = num >= 1 && num <= 9 ? num : 1;

  return {
    primary: colorMap[safeNumber],
    secondary: `Based on number ${safeNumber}`,
    meta: {
      name: safeName,
    },
  };
...
export function calculateLuckyNumber({ name, date }) {
  const safeName = name || "";

  const digits = date ? date.replaceAll("-", "").split("").map(Number) : [0];
...
export function calculateHoroscope({ name, rashi }) {
  const horoscopeMessages = {
    "Aries (Mesh)":
      "Today is favorable for taking initiative. Avoid impulsive decisions.",
    "Taurus (Vrishabha)":
      "Focus on stability and finances. Patience will bring rewards.",
    "Gemini (Mithun)":
      "Communication is your strength today. New ideas may emerge.",
    "Cancer (Karka)": "Emotional balance is important. Spend time with family.",
    "Leo (Simha)":
      "Leadership opportunities may arise. Stay humble and confident.",
    "Virgo (Kanya)":
      "Good day for planning and organizing. Health needs attention.",
    "Libra (Tula)":
      "Harmony in relationships improves. Avoid unnecessary arguments.",
    "Scorpio (Vrishchik)":
      "Trust your intuition. Transformation is on the horizon.",
    "Sagittarius (Dhanu)":
      "Travel or learning opportunities may appear. Stay optimistic.",
  ...
  const message = horoscopeMessages[rashi] || "Stay positive today.";
...
export function calculateSadeSati({ name, rashi }) {
  const activeRashis = [
    "Capricorn (Makara)",
    "Aquarius (Kumbha)",
    "Pisces (Meena)",
  ...
  const isActive = activeRashis.includes(rashi);

  let phase = "No Sade Sati";

  if (isActive) {
    if (rashi === "Capricorn (Makara)") phase = "Rising Phase (Initial)";
    else if (rashi === "Aquarius (Kumbha)") phase = "Peak Phase (Middle)";
    else if (rashi === "Pisces (Meena)") phase = "Setting Phase (Final)";
  }

  return {
    primary: isActive ? "Active" : "Not Active",
    secondary: isActive ? phase : "",
    meta: {
      name: name || "User",
      isActive,
    },
...
export function calculateDasha({ name, nakshatra }) {
  const nakshatras = [
  ...
  const selected =
    nakshatras.find((n) => n.name === nakshatra) || nakshatras[0];
...
export function calculateMarriageMatching({
  boyName,
  girlName,
  boyNakshatra,
  girlNakshatra,
  ...
  const safeBoy = boyIndex >= 0 ? boyIndex : 0;
  const safeGirl = girlIndex >= 0 ? girlIndex : 0;

  const diff = Math.abs(safeBoy - safeGirl);
...
export function calculateNameCompatibility({ nameOne, nameTwo }) {
  const letterValues = {
    A: 1,
    J: 1,
    S: 1,
  ...
  const numberOne = getNumber(nameOne);
  const numberTwo = getNumber(nameTwo);

  const diff = Math.abs(numberOne - numberTwo);
  const score = Math.max(50, 100 - diff * 10);

  let level = "Average Compatibility";
  if (score >= 80) level = "Excellent Compatibility ❤️";
  else if (score >= 65) level = "Good Compatibility 😊";
...
export function calculateLifePathNumber({ name, date }) {
  const safeName = name || "";

  const dobDigits = date ? date.replaceAll("-", "").split("").map(Number) : [0];
...
export function calculateNumerology({ name, date }) {
  const safeName = name || "";
...
export function calculateNakshatra({ name, date, time }) {
  ...
  const seed = day + month + year + strToNum(safeName) + timeVal;
...
export function calculateBirthChart({ name, date, time, place }) {
  const parsedDate = new Date(`${date}T${time}`);
  const isValidDate = !isNaN(parsedDate);
  ...
  return {
    sunSign: zodiacSigns[seed % 12],
    ascendant: zodiacSigns[(seed * 3) % 12],
    name: safeName || "User",
  };
}
export function calculateMoonSign({ name, date, time }) {
  const parsedDate = new Date(`${date}T${time}`);
  const isValidDate = !isNaN(parsedDate);
  ...
  return {
    moonSign: moonSigns[seed % 12],
    name: safeName || "User",
  };
}
export function calculateRashi({ name, date, time }) {
  const parsedDate = new Date(`${date}T${time}`);
  const isValidDate = !isNaN(parsedDate);
...
export function passingMarksCalculator({ totalMarks, passingPercentage }) {
...
export function examMarksCalculator({
  ...
  const score = correct * marksQ - wrongAnswers * negative;
...
export function scholarshipEligibilityCalculator({ percentage, familyIncome }) {
  const safePercentage = Number(percentage) || 0;
  const safeIncome = Number(familyIncome) || 0;

  const eligible = safePercentage >= 60 && safeIncome <= 800000;
...
export function neetRankPredictorCalculator({ percentage, totalCandidates }) {
  ...
  const predictedRank = Math.max(
    1,
    Math.round(((100 - safePercentage) * safeTotal) / 100),
  );
...
export function cutoffCalculator({ maths, physics, chemistry }) {
  ...
  const cutOff = safeMaths + safePhysics / 2 + safeChemistry / 2;
...
export function calculateSalary(ctc, basicPercent) {
  ...
  const taxableIncome = safeCTC - employeePF - standardDeduction;
...
export function calculateElectricalLoad({
  ...
  const totalWatts = lightLoad + fanLoad + acLoad + fridgeLoad + otherLoad;
...
export function calculateCement({ length, width, thickness, mixRatio }) {
...
export function calculateSand({ length, width, thickness, mixRatio }) {
  const t = thickness / 1000;
...
export function calculateAggregate({ length, width, thickness, mixRatio }) {
  const t = thickness / 1000;
...
export function calculateStaircase({ floorHeight, riserHeight, treadDepth }) {
...
export function calculatePaint({ length, width, height, coats, coverage }) {
...
export function calculateRCCSlab({ length, width, thickness, steelPercent }) {
  const l = Number(length) || 0;
  const w = Number(width) || 0;
...
export function calculateSteelWeight({ diameter, length, quantity }) {
...
export function calculateConcrete({ length, width, thickness, mixRatio }) {
  const l = Number(length) || 0;
  const w = Number(width) || 0;
  const t = (Number(thickness) || 0) / 1000;
...
export function calculateNps(monthly, annualRate, currentAge, retirementAge) {
  ...
  const corpus =
    r === 0 ? invested : monthly * ((Math.pow(1 + r, months) - 1) / r);
...
export function calculateXirr(flows) {
  ...
  for (let i = 0; i < 50; i++) {
    ...
    const derivative = (npv(rate + 0.00001, flows) - f) / 0.00001;
    ...
    if (Math.abs(next - rate) < 1e-8) return format(next * 100);
  ...
  for (let i = 0; i < 100; i++) {
    ...
    if (Math.abs(val) < 1e-6) return format(mid * 100);
...
export function calculateLoanEligibility({
  ...
  const eligibleLoan = (availableEMI * (pow - 1)) / (monthlyRate * pow);
...
export function calculatePrepayment({
  ...
  const calcEmi = (principal) => (principal * r * pow) / (pow - 1);
...
export function calculateBuyVsRentAdvanced({
  ...
  const futureValue = price * Math.pow(1 + growth / 100, tenure);

  const maintenanceCost = price * (maintenance / 100) * tenure;

  const buyingNetCost = totalEMI + maintenanceCost + downPayment - futureValue;

  const rentingNetCost = totalRent;

  const verdict =
    buyingNetCost < rentingNetCost ? "Buying is Better" : "Renting is Better";
  ...
  return {
    ...
    wealthDifference: Math.round(rentingNetCost - buyingNetCost),
    verdict,
    months,
  };
}
...
export function calculateCreditCardEMI({ principal, annualRate, months }) {
  const P = safeNumber(principal);
...
export function calculateROI({ investment, returns }) {
...
export function calculateBusinessLoan({ principal, annualRate, years }) {
...
export function calculateStartupValuation({ revenue, growthRate, multiple }) {
...
export function calculateEducationLoan({ principal, annualRate, years }) {
  const P = Number(principal) || 0;
  const rate = Number(annualRate) || 0;
  const tenure = Number(years) || 0;
