...
//   const rounded = Math.round(num * 100) / 100;
//   const value = Number.isInteger(rounded) ? Math.round(rounded) : rounded;
...
export function calculateTiles({
  ...
  if (!floorLength || !floorWidth) return null
  ...
  const tiles = Math.ceil(
    (floorArea / tileArea) * (1 + wastage / 100)
  )

  return {
    floorArea: format(floorArea),
    tileArea: format(tileArea),
...
export function calculateLoanEligibility({
  ...
  const safeLoan =
    Number.isFinite(eligibleLoan) ? eligibleLoan : 0
